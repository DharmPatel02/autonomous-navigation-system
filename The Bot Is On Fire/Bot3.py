...
#Finding shortest path using astar taking neighbors into consideration
...
def find_safe_path(grid, start, goal):
    #Finding out the sortest path
    size = len(grid)
    fire_positions = {(x, y) for x in range(size) for y in range(size) if grid[x, y] == 'F'}
    adjacent_fire_positions = set()

    for x, y in fire_positions:
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            neighbor = (x + dx, y + dy)
            if 0 <= neighbor[0] < size and 0 <= neighbor[1] < size:
                adjacent_fire_positions.add(neighbor)

    def is_safe(cell):
        return cell not in fire_positions and cell not in adjacent_fire_positions
    ...
    while open_set:
        ...
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if 0 <= neighbor[0] < size and 0 <= neighbor[1] < size and grid[neighbor[0], neighbor[1]] == 'O':
                if is_safe(neighbor):
...
def update(frame):
    ...
    path = find_safe_path(ship_layout, bot_position, button_position)
...
#plotting the variables
    ...
    if fire_stopped:
        ...
        plt.title("Bot heading towards neighbours considering neighbours")
